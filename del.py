...
CUBE = 30  # grid cell size used by the editor
...
class GeometryDash:
    ...
    def add_finish_wall(self):
        ...
        if objs:
            ...
            end_x = 900
        finish = {"type": "finish", "x": end_x, "y": GROUND_Y - FINISH_HEIGHT, "w": FINISH_WIDTH, "h": FINISH_HEIGHT}
    ...
    def add_obstacle_from_data(self, obj):
        ...
        if t in ("spike", "double_spike"):
            ...
            item = self.canvas.create_rectangle(x, y, x + w, y + h, fill=FINISH_COLOR, outline=TEXT_COLOR, width=3, tags="game")
        else:
    ...
    def handle_editor_click(self, x, y):
        ...
        if self.editor_tool == "block":
            # Blocks always snap to a fixed cube size and stack cleanly on
            # top of any blocks already placed in that column, instead of
            # taking a size from the click position (that was the cause
            # of "corrupted" block sizes).
